...
// a hand-written test config too.
...
// A test config the engine did NOT write (no marker). Its thresholds can't be
// safely baselined, so the coverage gate must stand down rather than risk a
// day-one-RED CI on a pre-existing high threshold.
function hasHandwrittenTestConfig(cwd, framework) {
  const files = framework === 'vitest' ? [...TEST_CONFIGS, ...VITE_CONFIGS] : TEST_CONFIGS;
