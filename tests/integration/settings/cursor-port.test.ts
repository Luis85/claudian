...
jest.mock('../../../src/core/providers/ProviderRegistry', () => ({
  ProviderRegistry: {
    getEnabledProviderIds: jest.fn().mockReturnValue(['cursor']),
    getRegisteredProviderIds: jest.fn().mockReturnValue(['cursor']),
...
describe('cursor tab port (integration)', () => {
  ...
  it('renders every registered cursor field and section through the registry', () => {
    ...
    // Tab content order matches shell's tabIds = [general, agentBoard, orchestrator, diagnostics, cursor].
    ...
    const fields = registry.getAllFields().filter((f) => f.tabId === 'cursor');
    ...
    const declaredSectionIds = registry
      .getSections('cursor', plugin.settings as never)
      .map((s) => s.id);
