...
- Product status: Claudian is a multi-provider product hosting four chat backends.
  ...
  - **Opencode** runs via the Opencode CLI server and supports send, stream, cancel, resume, history reload, image attachments, inline edit, `#` instruction mode, subagents, runtime-discovered slash commands, and Opencode-managed MCP. Plan **mode** runs (the session switches to Opencode's managed `plan` mode via the toolbar toggle / Shift+Tab), but the shared post-plan approval card is gated because the runtime does not emit `planCompleted`. Fork and rewind are gated.
