...
**Pick up at Phase E1.** Branch `feat/settings-overhaul-v3` is 28 commits ahead of `main`. Last commit `c5b6ff7` (text/folder fold in renderField).

Before starting any new task: `cd D:\Projects\claudian\.worktrees\feat\settings-overhaul-v3 && git status` (expect clean) and `git log --oneline main..HEAD | head -5` to confirm tip.

Sanity check baseline: `npm run test -- --selectProjects unit` → 5913 pass + 34 skipped; `npm run test -- --selectProjects integration` → 223 pass; `npm run typecheck && npm run lint && npm run build` → clean.

**Critical: branch MUST NOT merge to main until Phase E lands** — `general` is in `REGISTRY_TABS` but `fields/general.ts` registers zero fields, so the General settings tab currently renders 8 empty section headings in production. Phase E1–E7 (banner + provider rows + show-setup link) plus a still-unscheduled "Phase E.5" (port legacy General fields: language, display, conversations, content, input, environment) must complete before merge.

**Suggested next-session entry:** invoke `/superpowers:subagent-driven-development` against this file. Skill loads `using-superpowers` first, then dispatches Phase E task-by-task.

---

## Phase E entry-point checklist

Before dispatching E1:

- [ ] Read `src/features/settings/registry/fields/general.ts` (current state: 8 sections, 0 fields).
- [ ] Read `src/features/settings/ClaudianSettings.ts` `renderGeneralTab` (line 266+) to inventory legacy fields that must be ported alongside E.
- [ ] Confirm `ClaudianSettings` interface has `firstRunDismissed?: boolean` slot or that E1 adds it.
- [ ] Decide: align `general.ts` section ids with legacy renderer's section labels BEFORE E1, or roll into E1. Reviewer suggested doing it now to avoid churn in J1.

## Phase F entry-point checklist

Before dispatching F1:

- [ ] Decide how `SettingsCtx` exposes the plugin handle (5 stalled behaviors depend on this — see `src/features/settings/registry/SettingsField.ts` block comment above `SettingsCtx`).
- [ ] Confirm `ClaudianSettings.ts` shell passes the plugin into `ctx` at the `display()` `SettingsCtx` construction site (currently 3 fields: `settings`, `saveSettings`, `refresh`).
- [ ] Plan F4/F5 will replace placeholder dropdowns on agentBoard with custom resolver-aware widgets.

---

## Execution progress

Updated: 2026-05-30 (end of session 2 — polish pass complete). Method: subagent-driven (implementer → spec reviewer → quality reviewer per task).
...
| Polish | P1 Codex test depth | ✅ done | `d1e15c2` | Asserts appServerPath + apiKey field registrations |
| Polish | P2 Centralize obsidianDom | ✅ done | `5e965c7` | Folded addClass/removeClass/toggleClass/setText into shared polyfill |
| Polish | P3 Port-test helper | ✅ done | `809d54e` | `_portTestHelpers.ts`: mountSettingsShell + assertTabRendersRegistry; 7 tests ~20 lines each |
| Polish | P4 Hide agentBoardDefaultModel | ✅ done | `50c6a8b` | `visible` predicate gated on `agentBoardDefaultProvider` |
| Polish | P5 Document SettingsCtx gap | ✅ done | `567f0ad` | Block comment above SettingsCtx interface lists 4 blocked behaviors |
| Polish | P6 Fold renderField text/folder | ✅ done | `c5b6ff7` | Merged into single case block |
| E | E1–E7 first-run banner + provider rows | ⏳ pending | — | **Critical for merge.** General tab currently renders 8 empty headings until E lands. |
...
**Suite status after last commit (`c5b6ff7`)**: 5913 unit tests pass + 223 integration tests pass, 34 skipped, 0 fail. Lint clean (0 errors / 0 warnings). Typecheck clean. Build clean (styles.css 153.6 KB).
...
- `general.ts` section ids (`providers`, `appearance`, `chat`, `inlineEdit`, `agentMentions`, `performance`, `diagnostics`, `hotkeys`) do NOT match the plan's E-phase section names (`providers`, `language`, `display`, `conversations`, `content`, `input`, `environment`, `hotkeys`). Reviewer recommendation: realign before E1 so Phase E only adds fields and Phase J1 deletes legacy renderers cleanly. Either path is workable — pick during E1.
...
- C5 Agent Board `installCommonTemplatesButton.onClick` and C7 Diagnostics `copyDiagnosticLogs` / `clearDiagnosticLogs` buttons are no-op stubs with `TODO Phase F: invoke command '...' once SettingsCtx exposes plugin handle` comments. Phase F must extend `SettingsCtx` with a plugin handle (or equivalent command dispatcher) and wire the real `onClick` bodies. Polish P5 documented the gap inline at `src/features/settings/registry/SettingsField.ts` above the `SettingsCtx` interface.
...
- Polish P3 added `assertFunctionNames: ['expect', 'assertTabRendersRegistry', 'mountSettingsShell']` to `eslint.config.mjs` `jest/expect-expect` rule. Future custom assertion helpers must extend this list.

**Test infrastructure added (read before adding new settings tests):**
- `tests/setup/obsidianDom.ts` — HTMLElement.prototype polyfill: `createDiv`, `createEl`, `empty`, `addClass`, `removeClass`, `toggleClass`, `setText`. Idempotent. Import explicitly when needed; not loaded globally.
- `tests/integration/settings/_portTestHelpers.ts` — `mountSettingsShell({ tabId, tabContentIndex, providerEnabled? })` and `assertTabRendersRegistry(host, plugin, tabId)`. Pattern: each tab port test is ~20 lines.
- `tests/__mocks__/obsidian.ts` extended `Setting` class with `instances[]` static + `components[]` instance array; supports `addToggle`, `addText`, `addTextArea`, `addDropdown`, `addButton`. **Missing:** `addSlider` (legacy `renderGeneralTab` uses it; add to mock when Phase E ports the max-tabs slider).

**Dead-code inventory for J1** (verified end of session 2; J1 cleans these AFTER Phase E-H register the missing fields):

Files entirely unreachable in production:
- `src/features/settings/ui/AgentBoardSettingsSection.ts` (`renderAgentBoardSettingsSection`)
- `src/features/settings/ui/OrchestratorSettingsTab.ts` (`renderOrchestratorSettingsTab`)
- `src/features/settings/ui/LoggingSettingsSection.ts` (`renderLoggingSettingsSection`)
- `src/features/settings/ui/EnvironmentSettingsSection.ts` (`renderEnvironmentSettingsSection`)
- `src/features/settings/ui/QuickActionsSettingsTab.ts` (`renderQuickActionsSettingsTab`)
...
Methods of `ClaudianSettingTab` reachable only from other dead methods:
- `ClaudianSettings.ts` `renderGeneralTab`, `renderProvidersSection`, `renderHiddenProviderCommandSetting`, `renderCustomContextLimits`
- Helpers: `formatHotkey`, `openHotkeySettings`, `getHotkeyForCommand`, `addHotkeySettingRow`, `restartServiceForPromptChange`
- Type block: `ObsidianHotkey*` / `AppWithHotkeyInternals`

Imports to delete from `ClaudianSettings.ts` in J1:
- `Notice`, `Platform`, `Setting` (obsidian)
- `getHiddenProviderCommands`, `normalizeHiddenCommandList`
- `ProviderWorkspaceRegistry`
- `getAvailableLocales`, `getLocaleDisplayName` (keep `setLocale`)
- `Locale`, `TranslationKey`
- `formatContextLimit`, `parseContextLimit`, `parseEnvironmentVariables`
- `buildNavMappingText`, `parseNavMappings`
- `getProviderEnableUpdater`
- All 4 legacy renderer imports

Also reachable-only-from-tests after J1:
- `src/features/settings/keyboardNavigation.ts`
- `src/features/settings/providerEnableUpdaters.ts`
- `src/core/providers/commands/hiddenCommands.ts` `getHiddenProviderCommands` and `normalizeHiddenCommandList`
