...
These docs are the single-source content for product landing pages. They are authored once in the vault and adapted per channel (product website, GitHub README, in-app onboarding, etc.).
...
Pages never open with a feature name or a bullet list. They open with the reader in a recognizable situation — then show how the feature changes the outcome.

The Obsidian guarantee (local files, plain Markdown, no lock-in) is the backdrop, not a claim that needs proving. Specorator adds AI power on top of that guarantee, not instead of it.

Tone: trusted colleague showing you something useful, not a salesperson. Claims are grounded in what the feature actually does. Scope limits are stated honestly — what a feature does not do is as valuable to the reader as what it does.
...
Both personas are reached by scenario-led copy that grounds features in recognizable workflow moments.
...
One sentence. The workflow moment + what changes.
...
- Pattern: situation → friction → feature removes friction → outcome
- First paragraph always puts the reader in a recognizable moment
- One `<!-- screenshot: [description] -->` placed where a visual would be most persuasive
- Warm, direct — no superlatives, no buzzwords
...
2. **What it doesn't do** — honest scope limits. Short list. Prevents misaligned installs.
3. **Goes well with** — 2–4 wikilinks to related features, one-line context each.
...
type: feature
name: "Feature Name"
tagline: "One-sentence hook — workflow moment + what changes."
status: draft
personas:
  - knowledge-worker
  - pm
...
| `status` | Yes | `draft` → `ready` → `published` |
...
One per page. Placed inline in Layer 2 at the natural visual moment. Not a heading or a section — an inline comment.
...
>
> *Outcome paragraph.* What they have at the end. Optional — fold into feature paragraph if it reads naturally.
...
- Does not [specific behavior] — [one-line context]
```
...
| **Agent Kanban Board** | You have more work than one chat can hold. Ideas, specs, and delegated tasks need a lifecycle — from inbox to running to reviewed — without disappearing into chat history. |
...
- Does not inject the current selection automatically — write prompts that ask the agent to use what's already attached
- Not a slash-command or plugin-command system — quick actions are prompts, not Specorator commands
