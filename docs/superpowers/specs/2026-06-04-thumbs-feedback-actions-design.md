...
| `src/i18n/locales/en.json` (+ `de`, `es`, `fr`, `ja`, `ko`, `pt`, `ru`, `zh-CN`, `zh-TW`) | Four new keys under `chat.feedback`:<br>• `thumbsUp.label` — "Helpful"<br>• `thumbsUp.prompt` — "That response was helpful. Briefly note what worked so we keep it in mind going forward."<br>• `thumbsDown.label` — "Not helpful"<br>• `thumbsDown.prompt` — "That response wasn't helpful. Ask me one focused follow-up question to learn why I disagree before retrying." |
