...
| ~~PN-4~~ | — | ~~**Opencode plan mode**~~ — **CORRECTION (2026-06-03): already shipped.** `supportsPlanMode: true` (`opencode/capabilities.ts:7`); `permissionMode 'plan'` maps to the native `plan` mode (`opencode/modes.ts:124-147`, "Disallows all edit tools") applied via the ACP config path. The "gated" claim came from a stale CLAUDE.md line and is wrong. No action. | Opencode | verified against `252929d` |
...
| Plan mode | ✅ | ✅ | ✅ | ✅ | all shipped (Opencode plan mode confirmed wired) |
