...
/**
 * Whether a secret id was generated by Claudian's migration helpers. SecretStorage
 * ids are global across plugins, so auto-clearing on row deletion must be limited
 * to Claudian-owned ids — never an external/user-selected id another plugin owns.
 */
export function isClaudianGeneratedSecretId(id: string): boolean {
  return id.startsWith(`${CLAUDIAN_PREFIX}-`);
}
